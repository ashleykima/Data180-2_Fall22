...
dim(my_df)
```
# The dimension is 8 rows by 2 columns.
...
length(person)
```
# The total number of users is 8.

d) Write R code that counts the total number of users who prefer Twitter.
```{r}
length(grep("Twitter", preference))
```
# The total number of users who prefer Twitter is 4.

e) Write R code that returns what Gisele prefers.
```{r}
my_df[which(my_df$person == "Gisele"),]$preference
```
...
```
The 19th value in the fibonacci sequence is 4181.
